...
Four settings have to be correct before Windows is installed:
...
Everything else — memory profile, Resizable BAR, disabling unused devices — belongs on
a machine that already boots. An unstable XMP profile during installation presents as a
corrupt install; the same instability on a working system can be memtested.
