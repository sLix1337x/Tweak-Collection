...
Windows saves power on USB by letting devices go idle and suspending individual ports.
Waking one costs milliseconds and can delay or drop input reports. This tool disables
those mechanisms per driver and per device, recording the previous value of everything
it writes.
...
| **Interrupt threshold** | A latency knob rather than a power switch: an interrupt per transfer instead of coalescing them. |

Each is an independent toggle. The device list is built from a single
...
`-SelfTest` round-trips that path against a scratch registry key and cleans up after
itself, confirming the revert restores rather than guesses.

:::note[No console window, and its own icon]
`Run.vbs` has no window of its own: it elevates and starts PowerShell hidden in a
single `ShellExecute`, so nothing paints even for a frame. The script hides its own
console as a second line of defence. Started from an already-open terminal, that window
is left alone.

The title-bar and taskbar icon is embedded in the script as base64 rather than shipped
as a separate file, so the tool remains one file that can be dropped anywhere.

`.vbs` is what buys the zero-flash launch, and Microsoft has VBScript on a deprecation
path. If a future Windows drops it, the launcher is four lines of PowerShell away from
replacement; the script itself does not depend on it.
:::

## What it replaced

`USB_Static_Tweaks.reg` and `USB_Dynamic_Tweaks.bat`. Auditing those before rewriting
them turned up the same class of problem as
...
The two original files are not shipped. They are described in the tool's README as the
"before"; one of their values is an active downgrade and none can be undone.

## Notes

- A reboot is required for driver-level changes.
- Parts of the `Enum` tree are ACL'd to SYSTEM rather than Administrators, so a few keys
  can fail to write. Those are listed individually in the output pane; everything else
  still applies.
- On a laptop this costs idle battery. The tool detects a battery and reports it.
